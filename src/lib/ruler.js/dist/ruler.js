...
ruler.prototype.builder = function(){
  ...
  var positionRuler = function (curRuler, alignment) {
    curRuler.canvas.style.left = ruler.prototype.utils.pixelize(-((curRuler.canvas.width / 10) - curRuler.canvas.height));
    switch (alignment) {
      case 'top':
  ...
  var attachListeners = function (container, curRul) {
    var mousemove = function (e) {
      ...
      guideLine = constructGuideLine(curRul.dimension, e.clientX, e.clientY, e);
    ...
    curRul.canvas.addEventListener('mousemove', mousemove);
    curRul.canvas.addEventListener('mousedown', mousedown);
    curRul.canvas.addEventListener('mouseleave', mouseleave);
    curRul.clearListeners = function () {
      curRul.canvas.removeEventListener('mousemove', mousemove);
      curRul.canvas.removeEventListener('mousedown', mousedown);
      curRul.canvas.removeEventListener('mouseleave', mouseleave);
    }
  };

  var constructGuideLine = function (dimension, x, y, e, isSet) {
    var guideIndex;

    var guide = document.createElement('div'),
      guideStyle = dimension === VERTICAL ? 'rul_guideLineHor' : 'rul_guideLineVer',
  ...
  var constructGuide = function (dimension, x, y, e, isSet) {
    ...
    var guide = document.createElement('div'),
      guideStyle = dimension === VERTICAL ? 'rul_lineHor' : 'rul_lineVer',
      curDelta = dimension === VERTICAL ? CUR_DELTA_X : CUR_DELTA_Y;
    guide.title = 'Double click to delete';
    ruler.prototype.utils.addClasss(guide, ['rul_line', guideStyle]);
    guide = theRulerDOM.appendChild(guide);
...
ruler.prototype.rulerConstructor =  function(_canvas, options, rulDimension)
{
    ...
    var drawRuler = function (_rulerLength, _rulerThickness, _rulerScale){
        rulLength = canvas.width = _rulerLength * 2;
    ...
    var drawPoints = function () {
        var  pointLength = 0,
            ...
            drawText = false,
            lineLengthMax = 0,
            lineLengthMed = rulThickness / 2 - 1,
            lineLengthMin = (rulThickness / 2) + 5,
            gridSize = getGridSize(rulScale),
            rulerLength = rulLength*rulScale;
        ...
        for (var pos = 0; pos <= rulerLength; pos += 1) {
            ...
            if (delta % (200*gridSize) === 0) {
                pointLength = lineLengthMax;
                // label = delta + rulerLength/2 - 800;
                draw = true;
                drawText = true;
            }
            else if (delta % (100*gridSize) === 0) {
                pointLength = lineLengthMed;
                draw = true;
            }
            else if (delta % (20*gridSize) === 0) {
                pointLength = lineLengthMin;
                draw = true;
            }
            if (draw) {
                context.strokeStyle = '#d8dfe8'
                context.fillStyle = 'white'
                context.moveTo(pos + 0.5, rulThickness + 0.5);
                context.lineTo(pos + 0.5, pointLength +  0.5);
                // context.translate(pos + 0.5, pointLength +  0.5);
                // context.scale(2, 2);
                if (drawText) {
                  context.fillText(label+'', pos + 1.5, (rulThickness / 2) - 4);
                  label += 200;
                }
            }
        }
